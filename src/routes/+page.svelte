<script lang="ts">
  ...
  let randomColors = shuffleArrayInPlace(colors);

  onMount(() => {
    const htmlElement = document.documentElement;

    htmlElement.classList.add("opacity-100")
    ...
    htmlElement.style.setProperty("--color-6", randomColors[5])
  })
</script>
...
<main class="relative w-full">
  <div class="z-10 max-w-4xl mx-auto p-12 md:p-24 text-2xl relative">
  ...
  <button class="sticky bottom-20 top-10 button p-4 shadow border bg-white/10 border-white/40 backdrop-blur-sm cursor-pointer mx-auto block rounded-full my-8">Donate to the Hays Watterson Memorial Scholarship Fund</button>

  <section>
    <h2 class="text-center text-xl mt-8 mb-4">About Alexandra and Brian</h2>
  ...
  <section>
    <h2 class="text-center text-xl mt-8 mb-4">About the Scholarship</h2>
  ...
  <div class="z-0 right-side absolute right-0 left-1/2 h-full inset-0"></div>
  <div class="z-0 fixed inset-y-0 right-1/2 -mr-30 w-60 bg-transparent backdrop-blur-2xl"></div>
</main>
...
<style>
  ...
  .left-side {
    background: linear-gradient(-180deg, var(--color-1) 0%, var(--color-5) 84%);
  }

  .right-side {
    background: linear-gradient(-180deg, var(--color-3) 0%, var(--color-5) 84%);
  }
</style>
